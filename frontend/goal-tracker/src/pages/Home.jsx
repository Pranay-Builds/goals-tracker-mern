            ...
            return (
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <h2 className={`text-2xl font-semibold ${isCompleted ? "line-through text-gray-500" : "text-gray-900 dark:text-white"}`}>
                    {goal.title}
                  </h2>
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${new Date(goal.deadline) < new Date() ? "bg-red-500 text-white" : "bg-green-500 text-white"}`}>
                    {new Date(goal.deadline) < new Date() ? "Overdue" : "Active"}
                  </span>
                </div>

                <p className={`mb-4 ${isCompleted ? "text-gray-500" : "text-gray-700 dark:text-gray-300"}`}>
                  {goal.description}
                </p>

                <div className="flex justify-between items-center mb-4">
                  <p className="text-sm text-gray-600 dark:text-gray-400">📅 Started: {new Date(goal.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">⏳ Deadline: {new Date(goal.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                </div>
